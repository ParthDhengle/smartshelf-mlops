"""
SmartShelf — Dashboard Router
==============================
Dashboard, admin, and monitoring endpoints.
"""
...
from smartshelf.api.dependencies import clear_model_cache, get_db_engine
...
def _clear_model_cache_response() -> dict:
    clear_model_cache()
    return {"status": "cache_cleared", "message": "Model cache cleared."}
...
async def refresh_models():
    return _clear_model_cache_response()
...
async def get_kpis():
    ...
    try:
        rev_prof = pd.read_sql(
            """
            SELECT SUM(soi.line_total) as rev,
        ...
        counts = pd.read_sql(
            "SELECT (SELECT COUNT(*) FROM products) as p_cnt, (SELECT COUNT(*) FROM stores) as s_cnt",
            engine,
        )
        ...
        stockouts = pd.read_sql(
            "SELECT COUNT(*) as st, (SELECT COUNT(*) FROM inventory) as tot FROM inventory WHERE stock_on_hand <= 0",
            engine,
        )
        if int(stockouts.iloc[0]["tot"]) > 0:
            metrics["stockout_rate"] = round((int(stockouts.iloc[0]["st"]) / int(stockouts.iloc[0]["tot"])) * 100, 1)

        demand = pd.read_sql(
            """
            SELECT AVG(daily_units) as avg_d
            FROM (
                SELECT SUM(quantity) as daily_units
                FROM sales_order_items soi
                JOIN sales_orders so ON soi.order_id = so.order_id
                GROUP BY so.order_date, soi.product_id
            ) sub
            """,
            engine,
        )
        metrics["avg_demand"] = round(float(demand.iloc[0]["avg_d"] or 0), 1)

        promos = pd.read_sql(
            "SELECT COUNT(*) as act FROM promotions WHERE CURRENT_DATE BETWEEN start_date AND end_date",
            engine,
        )
        metrics["active_promos"] = int(promos.iloc[0]["act"])
    except Exception:
        pass
